...
\example{
    ...
    \Cref{fig:one-computer-concrete} illustrates this setup.
...
\example{
    ...
    and are processed at $25$ per hour.
    ...
    \begin{figure}
        ...
        \resizebox{\textwidth}{!}{
            \begin{tikzpicture}[node distance=1cm]
                % buffer nodes
                \node[buffer, font=\tiny] (B1) at (0,0) {$x_1(0)=100$};
                \node[fill=white, fill opacity=0.5, text opacity=1, font=\scriptsize] at (B1.north) {$B_1$};
                \node[buffer, font=\tiny] (B2) [below=of B1] {$x_2(0)=100$};
...
%    Our next example builds on \Cref{ex:one-computer-concrete}
%    by adding a second job class serviced by the same computer.
%    We continue to develop our example by fixing concrete values to the parameters of \Cref{ex:one-computer-two-classes-concrete}.
%
...
%    Building on \Cref{ex:example-one-computer-concrete},
...
%\label{sec:theory}
%
%\Cref{ex:one-computer-two-classes-concrete} has two classes of jobs,
